...
```bash
python scripts/estimate_camera.py --video "./example/example_video.mov" 
...
│   ├── __init__.py
│   ├── detection.py      # 检测组件
│   ├── segmentation.py   # 分割组件
│   ├── slam.py           # SLAM 组件
│   ├── hpe.py            # 人体姿态估计组件
│   ├── reprojection.py   # 重投影组件
│   └── evaluation.py     # 评估组件
├── backends/
│   ├── __init__.py
│   ├── detection/        # 检测后端 (vitdet, yolo...)
│   ├── segmentation/     # 分割后端 (sam, sam2...)
│   ├── slam/             # SLAM 后端 (droid, dpvo...)
│   └── hpe/              # HPE 后端 (vimo, hmr...)
├── builder.py            # 配置文件解析和 Pipeline 构建
└── hooks.py              # 预定义的常用 hooks
```
```
graph TB
    subgraph core [Core 层]
        PipelineData[PipelineData 数据容器]
        Component[Component 基类]
        Pipeline[Pipeline 引擎]
        IterPipeline[IterativePipeline]
    end
    ...
    subgraph components [Components 层]
        ...
        SLAM[SLAMComponent]
        HPE[HPEComponent]
        Reproj[ReprojectionComponent]
        Eval[EvaluationComponent]
    end
    
    subgraph backends [Backends 层]
        VitDet[VitDet Backend]
        SAM[SAM Backend]
        Droid[DroidSLAM Backend]
        Vimo[VIMO Backend]
    end
