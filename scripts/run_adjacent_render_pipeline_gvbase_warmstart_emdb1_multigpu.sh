...
# Adjacent Frame SMPL Rendering (GVHMR Base + Warmstart, EMDB1) - 多 GPU 并行
# EMDB1 仅评估 incam 指标（pa_mpjpe, mpjpe, pve, accel）
# 将序列均分到多张 GPU 上同时运行
# 使用示例：bash scripts/run_adjacent_render_pipeline_gvbase_warmstart_emdb1_multigpu.sh
# 自定义 GPU：GPU_IDS="0,1,2" bash scripts/run_adjacent_render_pipeline_gvbase_warmstart_emdb1_multigpu.sh
...
# 设置参数
CONFIG="configs/pipelines/gvhmr_base_warmstart_emdb1.yaml"
SPLIT=1
...
echo "============================================"
echo "多 GPU 并行运行 Pipeline (GVHMR Base + Warmstart, EMDB1)"
